            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-xl transition-all duration-300">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
                        ...
                        <!-- Tambah UMKM Button -->
                        <a href="{{ route('umkms.create') }}" class="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-600 hover:to-blue-700 text-white rounded-lg transition-all duration-300 transform hover:scale-105">
                    ...
                    <div class="relative overflow-x-auto rounded-xl shadow-lg transition-all duration-300">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" id="umkmTable">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200">
                                    ...
                                    <th scope="col" class="px-6 py-3 cursor-pointer" onclick="sortTable(5)">
                                        Status
                                    </th>
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors duration-200 transform hover:scale-[1.002]">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="px-3 py-1 rounded-full text-sm bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                                            {{ $umkm->category->nama_kategori ?? 'N/A' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">{{ $umkm->user->name ?? 'N/A' }}</td>
                                    <td class="px-6 py-4">{{ Str::limit($umkm->alamat, 30) }}</td>
                                    <td class="px-6 py-4">
                                        <span class="px-3 py-1 rounded-full text-sm 
                                            {{ $umkm->status_verifikasi == 'terverifikasi' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 
                                            ($umkm->status_verifikasi == 'pending' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' : 
                                            'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300') }}">
                                            {{ $umkm->status_verifikasi }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 flex items-center justify-center gap-2">
                                        <a href="{{ route('umkms.show', $umkm->id) }}" class="p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-all duration-300" title="Detail">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                            </svg>
                                        </a>
                                        <a href="{{ route('umkms.edit', $umkm->id) }}" class="p-2 text-yellow-600 hover:bg-yellow-100 rounded-lg transition-all duration-300" title="Edit">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                            </svg>
                                        </a>
                                        <form action="{{ route('umkms.destroy', $umkm->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin?')">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                        <div class="flex flex-col items-center justify-center py-8">
                                            ...
                                            <span class="text-lg">Tidak ada data UMKM</span>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
    ...
    <script>
        ...
        function searchTable() {
            ...
            for (let i = 1; i < tr.length; i++) {
                ...
                for (let j = 1; j < td.length - 1; j++) { // Skip nomor urut dan aksi
                    if (td[j]) {
        ...
        function sortTable(column) {
            ...
            rows.sort((a, b) => {
                ...
                if (column === 0) { // Numeric sorting untuk nomor urut
